...
Class LeagueApi
{
    private $LeagueAPIversion = "1.0.1";
    private $apiKey = ""; // API KEY

    public $error;
    public $notFound;
    
    function requestAPI($requestLink, $region){
        if($region == null){
            $region = null;
        }
        $setRegion = array(
        ...
        if(!array_key_exists($region, $setRegion)){
            $this->error = "Region not available";
            return $this->error;
        }
        
        $mainURL = 'https://'.$setRegion[$region].'.api.riotgames.com';
        return $mainURL."".$requestLink;
    }

    function getUserInformation($username, $region, $fullResponse = null){    
        $request = $this->requestAPI('/lol/summoner/v3/summoners/by-name/'.$username, $region);
        $this->executeRequest($request, $fullResponse);
        
        return $this->summonerInformation = $this->responseData;
    }
    

    function executeRequest($link, $fullResponse = null){
        $apiKey = $this->apiKey;
        $startRequest = curl_init();
        $headers = array(
        ...
        curl_setopt_array($startRequest, $request);

        $output = curl_exec($startRequest);
        if($fullResponse == true){
            $this->responseData = $this->getFullResponse($output);
        }
        else{
            $this->responseData = $this->getResponse($output);
        }

    }
    function getChampPicture($champId){
        $convertName = array(
            'Xin Zhao' => 'XinZhao',
            'Wukong' => 'MonkeyKing',
        ...
        $dirname =  '/proxy/8.19.1/img/champion/';
        //GET CHAMPION NAME
        $getChampName = $this->getChampName($champId);

        if(array_key_exists($getChampName, $convertName)){
            $getChampName = $convertName[$getChampName];
        }
        return '<img src="'.$dirname.str_replace(" ", "", $getChampName).'.png" class="rounded-circle">';
    }
    function getProfileIcon($iconID){
        $dirname =  '/proxy/8.19.1/img/profileicon/';

        return '<img src="'.$dirname.str_replace(" ", "", $iconID).'.png" width="150" height="150" class="rounded-circle float-right">';
    }
    function getRankBadge($rankID){
        $dirname =  '/proxy/8.19.1/img/badges/';

        return '<img src="'.$dirname.str_replace(" ", "", $rankID).'.png" width="150" height="150" class="rounded-circle">';
    }

    function getMatchInfo($matchID, $region, $fullResponse = null){
        $apiKey = $this->apiKey;
        $startRequest = curl_init();
        $headers = array(
            'Origin: https://developer.riotgames.com',
        ...
        $request = array(
            CURLOPT_URL => $this->requestAPI("/lol/match/v3/matches/".$matchID, $region),
            CURLOPT_RETURNTRANSFER => 1,
            CURLOPT_HTTPHEADER => $headers,
        );
        ...
        elseif($fullResponse == false){
            $this->responseData = $this->getResponse($output);
        }
    }
    function getAllMatches($summonerID, $region, $config, $fullResponse = null){
        $apiKey = $this->apiKey;
        $startRequest = curl_init();
        $headers = array(
            'Origin: https://developer.riotgames.com',
        ...
        $request = array(
            CURLOPT_URL => $this->requestAPI("/lol/match/v3/matchlists/by-account/".$summonerID, $region),
            CURLOPT_RETURNTRANSFER => 1,
            CURLOPT_HTTPHEADER => $headers,
        );
    ...
    function getUserRank($summonerID, $region){
        $getUserPosition = $this->requestAPI("/lol/league/v3/positions/by-summoner/".$summonerID, $region);
        $this->executeRequest($getUserPosition, true);

        return $this->userRank = $this->responseData;
    ...
    function getFullResponse($request){
        $jsonDecoded = json_decode($request, true);
